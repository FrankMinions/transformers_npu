...
class Qwen2FlashAttention2(Qwen2Attention):
    ...
    def forward(
        ...
        use_sliding_windows = self.config.use_sliding_window
    ...
    def _flash_attention_forward(
        ...
        if attention_mask is not None:
            ...
            if not use_sliding_windows:
                ...
                if not causal:
                    ...
                    atten_mask_npu = torch.triu(
                        torch.ones(max_seqlen_in_batch_q, max_seqlen_in_batch_k, dtype=torch.float16).bool().to(query_states.device), 1
                    )
                    head_num = query_states.shape[1]
                    attn_output_unpad = torch_npu.npu_fusion_attention(
                        query_states, key_states, value_states, head_num,
                ...
                raise ValueError(
                    f"`use_sliding_windows` must be False, but is {use_sliding_windows}"
                )
                # attn_output_unpad = flash_attn_varlen_func(
                #     query_states,
                #     key_states,
            ...
            if not use_sliding_windows:
                ...
                attn_output = torch_npu.npu_fusion_attention(query_states, key_states, value_states, head_num, "BSND",
                                                             keep_prob=1.0,
                                                             scale=softmax_scale)
            else:
                raise ValueError(
